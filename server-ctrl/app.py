#!/usr/bin/env python3
"""
Cultivee Hidroponia — API + Dashboard PWA
Servidor independente para controle de modulos ESP32-WROOM (reles, fases, ciclo).
"""
...
@app.route("/api/ctrl/<chip_id>/status")
@require_auth
...
@app.route("/api/ctrl/<chip_id>/relay")
@require_auth
...
@app.route("/api/ctrl/<chip_id>/phases")
@require_auth
...
@app.route("/api/ctrl/<chip_id>/save-config", methods=["POST"])
@require_auth
...
@app.route("/api/ctrl/<chip_id>/add-phase")
@require_auth
...
@app.route("/api/ctrl/<chip_id>/remove-phase")
@require_auth
...
@app.route("/api/ctrl/<chip_id>/reset-phases")
@require_auth
...
@app.route("/api/ctrl/<chip_id>/reset-wifi")
@require_auth
...
if __name__ == "__main__":
    models.init_db()
    log.info("Cultivee Hidroponia Server: http://localhost:5002")
