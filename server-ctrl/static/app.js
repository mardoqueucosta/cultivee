// =====================================================================
// Cultivee Hidroponia PWA — App Logic
// =====================================================================

const APP_VERSION = '1.0.1';
let token = localStorage.getItem("cultivee_ctrl_token");
let user = JSON.parse(localStorage.getItem("cultivee_ctrl_user") || "null");
let modules = [];
...
async function doLogin() {
    try {
        ...
        token = data.token;
        user = data.user;
        localStorage.setItem("cultivee_ctrl_token", token);
        localStorage.setItem("cultivee_ctrl_user", JSON.stringify(user));
        enterApp();
    } catch (e) {
...
function doLogout() {
    ...
    token = null;
    user = null;
    localStorage.removeItem("cultivee_ctrl_token");
    localStorage.removeItem("cultivee_ctrl_user");
...
function renderModules() {
    ...
    if (modules.length === 0) {
        list.innerHTML = `<div class="empty-state">
            ...
            <p style="font-size:0.8rem;color:var(--text-dim);margin-top:0.5rem">Conecte seu ESP32 na rede <strong>Cultivee-Hidro</strong> para configurar</p>
...
async function loadCtrlStatus(chipId) {
    try {
        const data = await api(`/api/ctrl/${chipId}/status`);
...
async function toggleRelay(chipId, device) {
    ...
    try {
        await api(`/api/ctrl/${chipId}/relay?device=${device}&action=toggle`);
    } catch (e) {
...
async function showConfigModal(chipId) {
    ...
    try {
        ...
        let data;
        try {
            data = await api(`/api/ctrl/${chipId}/phases?live=1`);
        } catch (e) {
            data = await api(`/api/ctrl/${chipId}/status`);
        }
...
async function saveConfig() {
    ...
    try {
        await api(`/api/ctrl/${configChipId}/save-config`, {
            method: "POST",
            body: data
        });
        closeConfigModal();
...
async function addPhase() {
    try {
        await api(`/api/ctrl/${configChipId}/add-phase`);
...
async function removePhase(idx) {
    ...
    try {
        await api(`/api/ctrl/${configChipId}/remove-phase?idx=${idx}`);
        showConfigModal(configChipId);
    } catch (e) {
        alert("Erro: " + e.message);
    }
...
async function resetPhases() {
    ...
    try {
        await api(`/api/ctrl/${configChipId}/reset-phases`);
        showConfigModal(configChipId);
    } catch (e) {
        alert("Erro: " + e.message);
    }
...
function checkPendingCode() {
    ...
    if (!code) {
        code = localStorage.getItem("cultivee_ctrl_pending_code");
    }

    if (code && token) {
        localStorage.removeItem("cultivee_ctrl_pending_code");
...
async function doAutoPair(code) {
    ...
    try {
        await api("/api/modules/pair", {
            method: "POST",
            body: { short_id: code, name: "Hidroponia" }
        });
        // Sucesso
        autoPairRetries = 0;
        loadModules();
...
if (token && user) {
    ...
    if (code) {
        localStorage.setItem("cultivee_ctrl_pending_code", code);
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (footer) {
        footer.textContent = `Cultivee Hidroponia v${APP_VERSION}`;
    }
});
