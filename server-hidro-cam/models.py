"""
Cultivee Hidroponia - Modelos do banco de dados (SQLite)
"""
...
DB_PATH = os.environ.get("DB_PATH", "data/cultivee-ctrl.db")
...
def register_module(chip_id, short_id, ip="", ssid="", rssi=0, uptime=0, free_heap=0, ctrl_data="{}"):
    conn = get_db()
    ...
    if existing:
        conn.execute(
            "UPDATE modules SET ip = ?, last_seen = ?, ssid = ?, rssi = ?, uptime = ?, free_heap = ?, ctrl_data = ? WHERE chip_id = ?",
            (ip, now, ssid, rssi, uptime, free_heap, ctrl_data, chip_id)
        )
    else:
        conn.execute(
            "INSERT INTO modules (chip_id, short_id, type, ip, last_seen, ssid, rssi, uptime, free_heap, ctrl_data) VALUES (?, ?, 'ctrl', ?, ?, ?, ?, ?, ?, ?)",
            (chip_id, short_id, ip, now, ssid, rssi, uptime, free_heap, ctrl_data)
        )
    conn.commit()
    conn.close()
...
def mark_activity(chip_id):
    ...
    conn = get_db()
    now = datetime.now().isoformat()
    # Usa tabela modules para guardar timestamp de atividade
    conn.execute(
